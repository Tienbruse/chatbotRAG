...
with open('/Users/Tienbruse/tmdt/Backend/company-llm/test/last_result.json', 'r', encoding='utf-8') as f:
...
rag_averages_adjusted = rag_averages.copy()
deepseek_averages_adjusted = deepseek_averages.copy()
rag_averages_adjusted['bertscore'] *= 100  
rag_averages_adjusted['deepseek_score'] /= 10  
deepseek_averages_adjusted['bertscore'] *= 100  
deepseek_averages_adjusted['deepseek_score'] /= 10  

result_data = {
    'RAG': rag_averages,
    'Deepseek': deepseek_averages
}
...
rag_values = [rag_averages_adjusted[m] for m in metrics]
deepseek_values = [deepseek_averages_adjusted[m] for m in metrics]
...
ax.set_title('So sánh các chỉ số trung bình giữa RAG và Deepseek')
ax.set_xticks(x)
